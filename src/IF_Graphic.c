...
void Set_Pallet_Shift(int16_t, int16_t);
...
int16_t G_STG68K_to_GR_Load(int8_t* , uint32_t , int16_t* , uint32_t , int8_t* , uint32_t , int16_t , int16_t , uint16_t, int8_t );
...
int16_t Get_PicImagePallet(uint8_t bNum)
{
	...
	if( (nCrtMod >= 12) && (nCrtMod <= 15) )return ret;	/* 65536色はパレット変更不可にする */
...
/* 機能		：	*/
/*===========================================================================================*/
void Set_Pallet_Shift(int16_t Col, int16_t Count)
{
	int16_t *pCG_PAL;
	int16_t i;
	int16_t temp[16];

	pCG_PAL = (int16_t *)0xE82000;	/* グラフィックのカラーテーブル０番  */

	if(Col < 16)
	{
		pCG_PAL += (Col * 16);	/* 目的のパレット移動 */
...
/* 機能		：	*/
/*===========================================================================================*/
void	G_Palette_HALF(void)
{
	int16_t i;
	uint16_t uGpal[256];

	Get_PicImagePalletTmp(&uGpal[0]);	/* 現在のパレットを退避 */
	for(i=0; i < 256; i++)
	{
		uGpal[i] = SetRGB(GetR(uGpal[i]) / 2, GetG(uGpal[i]) / 2, GetB(uGpal[i]) / 2);
	}
	Set_PicImagePalletTmp(&uGpal[0]);
...
//		Size = (pInfo->biHeight) * (pInfo->biWidth) * sizeof(uint16_t);
#ifdef DEBUG
//		printf("Head3(%d)=(%d)\n", i, Size );
//		KeyHitESC();	/* デバッグ用 */
#endif
...
int16_t CG_Mem_Convert_Type(uint16_t uListNum)
{
	...
	
	{
		...
		{
			Get_PicImageInfo( i, &uWidth, &uHeight, &uFileSize );	/* 画像の情報を取得 */
		}
...
//		KeyHitESC();	/* デバッグ用 */
#endif
		work_buf = (uint16_t*)MyMalloc( sizeof(uint16_t) * (X_MAX_DRAW * Y_MAX_DRAW) );	/* グラフィック解像度と同じ */
//		work_buf = (uint16_t*)MyMalloc( sizeof(uint16_t) * uHeight * uSize8x );	/* 元画像(横8の倍数)NG */
		if(work_buf == NULL)
		{
#ifdef DEBUG
			printf("error(0x%p):CG File%2d=%s\n", work_buf, i, g_stCG_LIST[i].bFileName );
//			KeyHitESC();	/* デバッグ用 */
#endif
			return -1;
		}
		...
		G_APIC_to_Mem(work_buf, pSrcBuf, uWidth, uHeight);	/* APIC画像を使いやすいBITMAP似のメモリに展開 */
...
#ifdef DEBUG
//		G_BitBlt_From_Mem(0, 0, 0, pSrcBuf, uWidth, uHeight, 0xFF, POS_LEFT, POS_TOP, i);
...
//		KeyHitESC();	/* デバッグ用 */
		...
		switch(g_stCG_LIST[i].bType)
		{
			case 1:	/* スプライトライク */
			{
				...
				p_sPath = Get_DataList_Path();
				sprintf(str, "%s\\cg\\OK_tmp_G%02d.G16", p_sPath, i );
#ifdef DEBUG
//				printf("CG_Mem_Convert_Type=0x%p\n", p_sPath);
#endif
...
//		KeyHitESC();	/* デバッグ用 */
		...
		//		G_PaletteSetZero();	/* 0番パレット変更 */
			...
			case 1:	/* スプライトライク(ファイルなし) */
			{
				...
				uint8_t str[256] = {0};
				p_sPath = Get_DataList_Path();
				sprintf(str, "%s\\cg\\OK_tmp_G%02d.P16", p_sPath, i );
...
void G_INIT(void)
{
	...
	ret = _iocs_vpage(0b0001);	/* グラフィック表示(page3:0n page2:0n page1:0n page0:0n) */
...
/* 機能		：	*/
/*===========================================================================================*/
void G_VIDEO_PRI(uint8_t bMode)
{
	switch(bMode)
	{
		default:
		case 0:	/* GR>TX>SP */
		*VIDEO_REG2 = Mbset(*VIDEO_REG2, 0x3FFF, 0b0010010011100100);	/* 優先順位 */
		break;
		case 1:	/* GR>SP>TX */
		*VIDEO_REG2 = Mbset(*VIDEO_REG2, 0x3FFF, 0b0001100011100100);	/* 優先順位 */
		break;
		case 2:	/* TX>GR>SP */
		*VIDEO_REG2 = Mbset(*VIDEO_REG2, 0x3FFF, 0b0010000111100100);	/* 優先順位 */
		break;
		case 3:	/* TX>SP>GR */
		*VIDEO_REG2 = Mbset(*VIDEO_REG2, 0x3FFF, 0b0001001011100100);	/* 優先順位 */
		break;
		case 4:	/* SP>GR>TX */
		*VIDEO_REG2 = Mbset(*VIDEO_REG2, 0x3FFF, 0b0000100111100100);	/* 優先順位 */
		break;
		case 5:	/* SP>TX>GR */
		*VIDEO_REG2 = Mbset(*VIDEO_REG2, 0x3FFF, 0b0000011011100100);	/* 優先順位 */
		break;
	}
}
/*===========================================================================================*/
...
void G_PaletteSetZero(void)
{
	...
	i =  G_COL_OFST;	/* カラーオフセット */
	
	/* 透過色 */
	GPALET( i++, SetRGB( 0,  0,  0));	/* 0:Black */
	/* １～１５を共通カラーとする */
	GPALET( i++, SetRGB(  1,  1,  1));	/* 1 */
	GPALET( i++, SetRGB_24BIT_TO_16BIT( 0xB2B2B2 ));	/* 2 */
	GPALET( i++, SetRGB_24BIT_TO_16BIT( 0xE5E5E5 ));	/* 3 */
	GPALET( i++, SetRGB_24BIT_TO_16BIT( 0xF3F3F3 ));	/* 4 */
	GPALET( i++, SetRGB_24BIT_TO_16BIT( 0xFFFFFF ));	/* 5 */
	GPALET( i++, SetRGB_24BIT_TO_16BIT( 0x207546 ));	/* 6 */
	GPALET( i++, SetRGB( 15,  0,  0));	/* 7 */
	GPALET( i++, SetRGB( 15,  0, 15));	/* 8 */
	GPALET( i++, SetRGB(  0, 15, 15));	/* 9 */
	GPALET( i++, SetRGB( 30,  1,  1));	/* A */
...
#define G_BG		(0x00)
#define G_BLACK		(0x01)
#define G_D_GRAY	(0x02)
#define G_GRAY		(0x03)
#define G_D_WHITE	(0x04)
#define G_WHITE		(0x05)
#define G_D_GREEN	(0x06)
#define G_D_RED		(0x07)
#define G_D_BLUE	(0x08)
#define G_INDIGO	(0x09)
#define G_RED		(0x0A)
#define G_GREEN		(0x0B)
#define G_YELLOW	(0x0C)
#define G_BLUE		(0x0D)
#define G_L_PINK	(0x0E)
...
int32_t G_CLR(void)
{
	...
	_iocs_g_clr_on();			/* グラフィックのクリア(スプライトまでOFFにするから要注意) */
...
#ifdef DEBUG
//	printf("G_Load_Mem(0x%p)=(%d(%d),%d)\n", pSrcGR, uWidth, uSize8x, uHeight );
//	KeyHitESC();	/* デバッグ用 */
#endif
...
{	
	...
	int32_t nCrtMod;
	nCrtMod = _iocs_crtmod(-1);
	if( (nCrtMod >= 12) && (nCrtMod <= 15) )return -3;	/* 65536色はパレット変更不可にする */
...
void G_APIC_to_Mem(uint16_t *pSrcBuf, uint16_t *pDstBuf, uint16_t uWidth, uint16_t uHeight)
{
	...
	{
		pBuf = pSrcBuf + (y * X_MAX_DRAW);	/* srcが512ｘ512を想定(pointer+実数) */		
...
//			printf("(%d, %d(%x))=0x%p\n", x, y, *pSrc, pDstGR);
			...
			if( ((posX + x) >= X_MIN_DRAW) && ((posX + x) < X_MAX_DRAW) )
			{
//				*pDstGR = (*pSrc >> 4) & 0xF;	/* 256色の場合 */
				*pDstGR = pPAL[(*pSrc >> 4) & 0xF];	/* 65536色の場合 */
				pDstGR++;
//				*pDstGR = (*pSrc) & 0xF;	/* 256色の場合 */
				*pDstGR = pPAL[(*pSrc) & 0xF];	/* 65536色の場合 */
				pDstGR++;
...
/* 機能		：	*/
/*===========================================================================================*/
int16_t G_STG68K_to_GR_Load(int8_t* pPTN, uint32_t nPTN_Size, int16_t* pCOL, uint32_t nCOL_Size, int8_t* pPAL, uint32_t nPAL_Size, int16_t posX, int16_t posY, uint16_t uArea, int8_t bMode )
{
	int16_t	ret = 0;
	...
	uint32_t	uWidth, uHeight;
	uint16_t	uSize8x = 0;
	uint16_t	nCol, nCol16b;
	uint16_t	nPalBuf[32][16];
...
#ifdef DEBUG
//	printf("nPTN_Size %d uWidth %d uHeight %d uSize8x %d\n", nPTN_Size, uWidth, uHeight, uSize8x );
#endif
	
	for(y = 0; y < 32; y++)	/* 0番はスプライト用 */
	{
		pSrc16 = (int16_t *)((uint32_t)pCOL + (0x20 * y));

		for(x = 0; x < 16; x++)
		{
			// ソースの0bitから15bitまで順番にどこのビットに移動したいか（0 → , 1 → , ..., 15 → ）
			...
			nPalBuf[y][x] = reorder_bits(*pSrc16, reverse_order);
			G_PaletteSet((y * 16)  + x, nPalBuf[y][x] );
...
//	printf("\n");
	...
	for(y = 0; y < uHeight; y++)
	{
		...
		pSrc = pPTN + ((y % 16) * 0x08) + ((y / 16) * 0x800);
		
		for(x = 0; x < uSize8x; x+=16)
		{
			nCol16b = Mmax( 0, pPAL[ (0x10 * (y / 16)) + (x / 16) ] );
			nCol = 16 * nCol16b;
		
			if( ((posX + x) >= X_MIN_DRAW) && ((posX + x) < X_MAX_DRAW) )
			{
				/* 0:1-2 */
//				*pDstGR = ((*pSrc) & 0xF) + nCol;	/* 256色の場合 */
				*pDstGR = nPalBuf[nCol16b][((*pSrc) & 0xF)];
				pDstGR++;
//				*pDstGR = ((*pSrc >> 4) & 0xF) + nCol;	/* 256色の場合 */
				*pDstGR = nPalBuf[nCol16b][((*pSrc >> 4) & 0xF)];
				pDstGR++;
